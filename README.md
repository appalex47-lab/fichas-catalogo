...
### Documento de reglas para Regulatorio

El botón **Reglas** (el libro junto al engrane) abre el documento de reglas de construcción de las fichas. Se puede descargar en HTML (Word también lo abre) o imprimir como PDF.

- **Siempre al día:** se genera con el mismo código que valida las fichas. Las estructuras y plantillas por categoría, las listas de términos del semáforo de claims, las frases fijas, los bloqueos de Magento, las reglas de exportación y el uso de IA se leen de ahí, y los ejemplos de formato se calculan al generarlo. Refleja también la configuración vigente: siglas en mayúsculas y estructura de metas.
- **Origen y efecto de cada regla:** cada una indica de dónde viene (prompt aprobado, agencia de SEO, NOM-141 en cosméticos o criterio de la herramienta) y si la herramienta **bloquea**, **avisa**, **corrige** o solo **documenta**.
- **Huella:** un código de 8 caracteres que cambia cuando cambia cualquier regla, plantilla, lista de términos o estructura de meta. Permite comprobar que el documento firmado corresponde a la versión en uso.
- **Pendientes visibles:** el fundamento normativo de las categorías distintas de cosméticos queda marcado como «Por completar por Regulatorio». Los numerales de la NOM-141 también los debe confirmar Regulatorio.
- **Alcance:** no es asesoría legal y no está aprobado hasta que lo firme Regulatorio. Trae un bloque para nombre, cargo, fecha, firma y comentarios.
...
Abre `index.html` en el navegador, o publícala con GitHub Pages (Settings, Pages, rama `main`, carpeta raíz). Para que funcione, `index.html`, `styles.css`, `logic.js`, `assist.js`, `ai.js`, `rules.js` y `app.js` deben estar en la misma carpeta.
...
| `rules.js` | Genera el documento de reglas de construcción para Asuntos Regulatorios, leyendo las reglas del propio código. Funciona en navegador y en Node |
...
| `tests/*.test.js` | Pruebas automáticas de `logic.js`, `assist.js`, `ai.js` y `rules.js`. No las usa la página |
...
La página solo necesita `index.html`, `styles.css`, `logic.js`, `assist.js`, `ai.js`, `rules.js` y `app.js`. Los demás archivos sirven para documentar, proteger y mantener el código.
