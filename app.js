...
function fieldHTML(fd) {
  ...
  const label = `<label for="f-${fd.key}">${esc(fd.label)}${fd.req ? '<span class="req" title="Requerido por la estructura">*</span>' : ''}</label>`;
  let ctl;
  if (fd.type === 'select') {
...
function renderAlerts(r) {
  ...
  if (m && r.title.title.toLowerCase().split(m).length - 1 > 1) parts.push(noteList('note--warn', 'Información duplicada', ['La marca aparece más de una vez en el título.']));
...
function panelTitle(r) {
  ...
  const strip = t.segs.map(s => {
    ...
    return `<div class="seg ${s.state === 'partial' ? 'seg--partial' : ''}"><div class="seg-text">${esc(s.text)}</div><div class="seg-label">${esc(s.label)}</div>${s.missing.length ? `<div class="seg-flag">Falta: ${esc(s.missing.join(', '))}</div>` : ''}</div>`;
  }).join('');
...
const FILTERS = { todos: 'Todos', faltantes: 'Con datos faltantes', bloqueados: 'Magento bloqueado', lenguaje: 'Revisar lenguaje' };
function statusOf(r) {
...
function evalItem(it, keep) {
  ...
  return { res, tag: statusOf(res), lang: L.claim.length + L.vacio.length + L.amber.length };
}
const tagsHTML = e => `<span class="tag tag--${e.tag[0]}">${esc(e.tag[1])}</span>` + (e.lang ? ` <span class="tag tag--warn">Revisar lenguaje</span>` : '');
const passFilter = (e, f) => f === 'todos' || (f === 'faltantes' && e.res.title.missing.length) || (f === 'bloqueados' && e.res.mg.blocked) || (f === 'lenguaje' && e.lang);
  ...
  body.innerHTML =
    ...
    (rows.length ? `<div class="tablewrap"><table>
      ...
      <tbody>${shown.map(({ it, e }) => `<tr><td>${esc(it.sku || '')}</td><td>${e.res.c.emoji} ${esc(e.res.c.name)}</td><td class="t">${esc(e.res.title.title)}</td>
        <td>${tagsHTML(e)}</td>
        <td class="ops"><button class="btn btn--quiet" data-edit="${it.id}">Editar</button><button class="btn btn--quiet" data-del="${it.id}">Quitar</button></td></tr>`).join('')}</tbody></table></div>`
...
function exportRows() {
  const keep = keepSet();
  const head = ['SKU', 'Categoría detectada', 'Título', 'Descripción Merchant Center', 'Descripción Magento (HTML)', 'Requiere receta médica', 'Estado', 'Datos faltantes', 'Meta title', 'Meta description', 'Alt imagen principal'];
  const rows = state.lote.map(it => {
    ...
    return [it.sku || '', r.c.name, r.title.title, r.mc.text,
      ...
      r.mg.meta.receta || 'Sin declarar', e.tag[1] + (e.lang ? '; Revisar lenguaje' : ''), miss, r.meta.mt.text, r.meta.md.text, r.meta.alt.text];
  });
  return [head, ...rows];
}
...
document.addEventListener('click', e => {
  ...
  if (t.dataset.edit) {
    ...
    if (!it) return;
    state.cat = it.cat; state.v = { ...it.v }; state.sku = it.sku || ''; state.img = it.img || '';
...
$('#add').addEventListener('click', () => {
  ...
  state.lote.push({ id: uid(), sku: state.sku.trim(), img: state.img.trim(), cat: state.cat, v });
  store.set('fichas.lote.v1', state.lote);
  state.v = {}; state.sku = ''; state.img = '';
  renderForm(); renderOutputs(); renderLote();
  toast('Producto agregado al lote');
});
$('#clear').addEventListener('click', () => { state.v = {}; state.sku = ''; state.img = ''; renderForm(); renderOutputs(); });
...
$('#extract').addEventListener('click', () => {
  ...
  state.cat = res.cat; state.v = { ...res.fields };
  renderCats(); renderForm(); renderOutputs();
...
function renderBulk() {
  ...
  const rowHTML = x => {
    ...
    return `<tr><td>${row.n}</td><td>${esc(row.sku)}</td><td>${x.res.c.emoji} ${esc(x.res.c.name)}</td><td class="t">${esc(x.res.title.title)}${row.note ? `<div class="hint">Sin asignar: ${esc(row.note)}</div>` : ''}</td><td>${tagsHTML(x)}</td></tr>`;
  };
  box.innerHTML = `
    ...
    <div class="actions">
      <button class="btn btn--primary" data-bulk="add" ${ready.length ? '' : 'disabled'}>Agregar ${ready.length} al lote</button>
      <button class="btn btn--quiet" data-bulk="discard">Descartar archivo</button>
    </div>`;
}
function bulkAdd() {
  ...
  ok.forEach(r => {
    const item = { id: uid(), sku: r.sku, img: r.img || '', cat: r.cat, v: r.v };
...
function expOpts() { return { attr: state.exp.attr, incFaltantes: state.exp.incF, excLenguaje: state.exp.excL }; }
...
/* Inicio */
$('#exp-attr').value = state.exp.attr; $('#exp-enc').value = state.exp.enc; $('#exp-incf').checked = state.exp.incF; $('#exp-excl').checked = state.exp.excL;
renderMetaCfg();
renderCats(); renderForm(); renderOutputs(); renderLote(); renderBulk(); setTab('titulo');
})();
