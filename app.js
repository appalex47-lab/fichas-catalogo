...
const state = {
  ...
  aiFlags: {}, aiSuggest: [], asst: [], aiModel: store.get('fichas.aimodel.v1', ''),
...
function renderBulk() {
  ...
  box.innerHTML = `
    ...
    <div class="actions">
      ${aiCand.length && !b.running ? `<button class="btn btn--ai" data-bulk="ai">🤖 Completar ${aiCand.length} filas con IA</button>` : ''}
...
function panelMeta(r) {
  const m = r.meta;
  return (m.confirmed ? '' : `<div class="note note--info"><p>La estructura de meta de esta categoría no está confirmada. En el CSV de Magento, meta_title, meta_description y short_description van vacías. Configúrala en <button class="linkbtn" data-setopen="magento">Ajustes, pestaña Magento</button>.</p></div>`)
    + metaWarns(m)
...
async function asstSend(question) {
  ...
  if (!key) { state.asst.push({ role: 'error', content: 'Falta la llave de Cohere. Agrégala en Ajustes (engrane), pestaña IA.' }); renderAsst(); return; }
...
renderMetaCfg();
renderCats(); renderForm(); renderOutputs(); renderLote(); renderBulk(); renderAiSuggest(); renderAsst(); setTab('titulo');
})();
