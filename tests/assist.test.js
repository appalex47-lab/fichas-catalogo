...
test('documento de reglas: acción permitida y respuesta local', () => {
  assert.equal(S.parseAction('abrir:reglas', 'med').label, 'Abrir el documento de reglas');
  const r = S.localAnswer('¿Dónde veo el documento de reglas para Regulatorio?', base);
  assert.match(r.text, /botón «Reglas»/);
  assert.deepEqual(r.actions, ['abrir:reglas']);
  assert.ok(S.ACTION_IDS.includes('abrir:reglas'));
});
