...
const STATIC_ACTIONS = {
  ...
  'abrir:reglas': 'Abrir el documento de reglas'
};
...
const INTENTS = [
  ...
  ['rules', q => /(documento|manual).*(regla|regulator|construccion)|reglas de construccion|documento regulatorio|para regulatorio|que reglas (sigue|usa|aplica)|auditoria|huella/.test(q), () => ({ text: 'El botón «Reglas» (el libro junto al engrane) abre el documento de reglas de construcción de fichas. Se genera con el mismo código que valida las fichas, así que siempre está al día. Incluye principios, estructuras y plantillas por categoría, listas de términos, leyendas fijas, bloqueos y reglas de exportación, indica el origen de cada regla y trae un bloque de firma para Regulatorio. Puedes descargarlo o imprimirlo como PDF.', actions: ['abrir:reglas'] })],
